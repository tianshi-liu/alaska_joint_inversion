...
max_jobs=5
src_file=src_rec_noise/sources_batch.dat
current_dir=`pwd`
...
while read line; do
  ...
  echo ${src_code}
  # set up slurm script
  sed -i "/^#SBATCH --time=/c\#SBATCH --time=00:80:00" ${slurm_script}
  ...
  if ${is_restart}; then
    ...
    if grep -q "^${sta_code}" src_rec_noise/sources_ctrlgrp.dat; then
    ## use this during point-spreading test
    #if grep -q "^${sta_code}" src_rec_noise/sources_batch.dat; then
  ...
  sed -i "/^go_combine_kernels=/c\go_combine_kernels=true" ${slurm_script}
